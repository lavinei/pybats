...
def analysis_dlm(Y, X, k, forecast_start, forecast_end, nsamps=500, holidays = [],
                 seasPeriods = [7], seasHarmComponents = [[1,2,3]],
                 latent_factor = None,
                 mean_only = False, dates=None, ret=['seasonal_weekly'], new_latent_factors = None,
                 ntrend=2, prior_length=0, **kwargs):
    """
...
def analysis(Y, X,
    ...
    if model_prior is None:
        mod = define_normal_dlm(Y, X, prior_length=prior_length, ntrend=ntrend, nhol=nhol, nlf=nlf,
                                 seasPeriods=seasPeriods, seasHarmComponents=seasHarmComponents,
                                 **kwargs)
    else:
        mod = model_prior
